...
var pickFolderStep = {
  ...
  async run(ctx) {
    ...
    if (process.platform === "win32") {
      ...
      const r2 = await execa("powershell", ["-NoProfile", "-STA", "-Command", ps], { reject: false, windowsHide: true });
      picked = (r2.stdout ?? "").trim();
    } else if (process.platform === "darwin") {
      const r2 = await execa("osascript", ["-e", 'POSIX path of (choose folder with prompt "\uB808\uD3EC\uB97C \uD074\uB860\uD574 \uB458 \uD3F4\uB354")'], { reject: false });
      picked = (r2.stdout ?? "").trim().replace(/\/$/, "");
    } else {
      const r2 = await execa("zenity", ["--file-selection", "--directory", "--title=\uB808\uD3EC\uB97C \uD074\uB860\uD574 \uB458 \uD3F4\uB354"], { reject: false });
      picked = (r2.stdout ?? "").trim();
    }
    if (!picked) {
